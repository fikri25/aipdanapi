...
class Aspek_investasi extends CI_Controller {
	function __construct() {
		...
		$this->load->model('tahunan_model/aspek_investasi_th_model');
	...
	public function index(){
        ...
        $data['sum'] = $this->aspek_investasi_th_model->getdataindex('aset_investasi_front_sum', 'row_array', 'INVESTASI');
        // print_r($data);exit;
        $data['data_penempatan_investasi_ket_thn'] = $this->aspek_investasi_th_model->get_ket('ket_lkai_penempatan_investasi');
        $data['bread'] = array('header'=>'Aspek Investasi', 'subheader'=>'Aspek Investasi');
        $data['view']  = "tahunan/aspek_investasi/index-investasi";
    ...
    public function hasil_investasi(){
        ...
        $data['sum'] = $this->aspek_investasi_th_model->getdataindex('aset_investasi_front_sum', 'row_array', 'HASIL INVESTASI');
        // print_r($data);exit;
        $data['data_penerimaan_investasi_ket_thn'] = $this->aspek_investasi_th_model->get_ket('ket_lkai_penerimaan_investasi');

        $data['bread'] = array('header'=>'Aspek Investasi', 'subheader'=>'Aspek Investasi');
        $data['view']  = "tahunan/aspek_investasi/index-hasil-investasi";
        $this->load->view('main/utama', $data);
    }
    ...
    public function beban_investasi(){
        ...
        $data['sum'] = $this->aspek_investasi_th_model->getdataindex('aset_investasi_front_sum', 'row_array', 'BEBAN INVESTASI');
        // print_r($data);exit;
        $data['data_beban_investasi_ket_thn'] = $this->aspek_investasi_th_model->get_ket('ket_lkai_beban_investasi');
        $data['bread'] = array('header'=>'Aspek Investasi', 'subheader'=>'Aspek Investasi');
        $data['view']  = "tahunan/aspek_investasi/index-beban-investasi";
        $this->load->view('main/utama', $data);
    }
    ...
    public function karakteristik_invest(){
        ...
        // print_r($data);exit;
        $data['data_karakteristik_investasi_ket_thn'] = $this->aspek_investasi_th_model->get_ket('ket_lkai_karakteristik_investasi');
        $data['bread'] = array('header'=>'Aspek Investasi', 'subheader'=>'Aspek Investasi');
        $data['view']  = "tahunan/aspek_investasi/index-karakteristik-investasi";
        $this->load->view('main/utama', $data);
    }
    ...
    function get_index($mod){
        switch($mod){
            ...
            case 'index-beban-investasi':
                $data['iduser'] = $this->input->post('iduser');
                $data['data_beban'] = $this->beban_investasi_semester();
                $data['sum'] = $this->aspek_investasi_th_model->getdataindex('aset_investasi_front_sum', 'row_array', 'BEBAN');
    ...
    function get_form($mod){
        ...
        switch($mod){
            ...
            case 'karakteristik_investasi':
                ...
                $data['bread'] = array('header'=>'Aspek Investasi', 'subheader'=>'Aspek Investasi');
                $data['view']  = "tahunan/aspek_investasi/input-karakter-invest";
    ...
    public function save_keterangan(){
        ...
        if ($ext=="pdf" OR $ext=="doc" OR $ext=="docx" OR $ext=="") {
            ...
            $this->aspek_investasi_th_model->delete_ket($jns_lap,$iduser);
            $this->aspek_investasi_th_model->insert_ket($data);
    ...
    public function get_file(){
        ...
        $get_db = $this->aspek_investasi_th_model->get_by_id_ket($id);
    ...
    public function aset_investasi_semester(){
        ...
        foreach ($invest as $k => $v) {
           ...
           $pers_nom = ($v['saldo_akhir_thn']!=0)?(($v['saldo_akhir_thn_lalu'] - $v['saldo_akhir_thn'])/$v['saldo_akhir_thn'])*100:0;
            ...
            $array[$k]['child'] = array();
            if($v['type'] == "PC"){
                $childinvest = $this->aspek_investasi_th_model->getdataindex('aset_investasi_front_lv3','result_array', $v['id_investasi'], $param_jenis);
    ...
    public function beban_investasi_semester(){
        ...
        foreach ($invest as $k => $v) {
            ...
            $array[$k]['saldo_akhir_thn_lalu'] = (isset($v['saldo_akhir_thn_lalu']) ? $v['saldo_akhir_thn_lalu'] : 0) ;
            $array[$k]['rka'] = $v['rka'];
            $pers_rka= ($v['rka']!=0)?($v['saldo_akhir_thn_lalu']/$v['rka'])*100:0;
            $pers_rka = (!is_nan($pers_rka) && !is_infinite($pers_rka) ? $pers_rka : '0,00');
            $array[$k]['pers_rka'] = $pers_rka;
            $array[$k]['type'] = $v['type'];
            $array[$k]['nominal'] = $v['saldo_akhir_thn_lalu'] - $v['saldo_akhir_thn'];
            ...
            if($v['type'] == "PC"){
                ...
                foreach ($childinvest as $key => $val) {
                    ...
                    $pers_rka2= ($val['rka']!=0)?($val['saldo_akhir_thn_lalu']/$val['rka'])*100:0;
                    $pers_rka2 = (!is_nan($pers_rka2) && !is_infinite($pers_rka2) ? $pers_rka2 : '0,00');
                    $array[$k]['child'][$key]['pers_rka'] = $pers_rka2;
                    $array[$k]['child'][$key]['nominal'] = $val['saldo_akhir_thn_lalu'] - $val['saldo_akhir_thn'];
                    $min2 = $val['saldo_akhir_thn_lalu'] - $val['saldo_akhir_thn'];
                    $pers_nom2 = ($val['saldo_akhir_thn']!=0)?(($val['saldo_akhir_thn_lalu'] - $val['saldo_akhir_thn'])/$val['saldo_akhir_thn'])*100:0;
                    $pers_nom2 = (!is_nan($pers_nom2) && !is_infinite($pers_nom2) ? $pers_nom2 : '0,00');
                    $array[$k]['child'][$key]['persentase'] = $pers_nom2;
                    $array[$k]['child'][$key]['type'] = $val['type'];
    ...
    public function karakteristik_invest_semester(){
        $param_jenis = 'INVESTASI';
        $array = array();
        $invest = $this->aspek_investasi_model->getdataindex('karakteristik_invest_lv1','result_array', $param_jenis);
        ...
        foreach ($invest as $k => $v) {
            ...
            $array[$k]['child'] = array();
            if($v['type'] == "PC"){
                $childinvest = $this->aspek_investasi_model->getdataindex('karakteristik_invest_lv2','result_array', $v['id_investasi'], $param_jenis);
                foreach ($childinvest as $key => $val) {
                    $array[$k]['child'][$key]['id'] = $val['id'];
                    $array[$k]['child'][$key]['id_investasi'] = $val['id_investasi'];
                    $array[$k]['child'][$key]['jenis_investasi'] = $val['jenis_investasi'];
    ...
    function cetak($mod=""){
        ...
        switch($mod){
            case "penempatan_investasi_cetak":
                ...
                $data['data_invest'] = $this->aset_investasi_semester();
                $data['sum'] = $this->aspek_investasi_th_model->getdataindex('aset_investasi_front_sum', 'row_array', 'INVESTASI');
                $template  = $this->load->view('tahunan/aspek_investasi/cetak/index-penempatan-investasi-cetak', $data,true);  
                // print_r($data);exit;
            ...
            case "penerimaan_hasil_investasi_cetak":
                ...
                $data['data_hasil'] = $this->hasil_investasi_semester();
                $data['sum'] = $this->aspek_investasi_th_model->getdataindex('aset_investasi_front_sum', 'row_array', 'HASIL INVESTASI');
                $template  = $this->load->view('tahunan/aspek_investasi/cetak/index-penerimaan-hasil-investasi-cetak', $data,true);  
            ...
            case 'beban_investasi_cetak':
                ...
                $data['data_beban'] = $this->beban_investasi_semester();
                $data['sum'] = $this->aspek_investasi_th_model->getdataindex('aset_investasi_front_sum', 'row_array', 'BEBAN');
                $template  = $this->load->view('tahunan/aspek_investasi/cetak/index-beban-investasi-cetak', $data,true);  
            ...
            case 'karakteristik_investasi_cetak':
                $data['iduser'] = $this->input->post('iduser');
                $data['data_karakter'] = $this->karakteristik_invest_semester();
                $template  = $this->load->view('tahunan/aspek_investasi/cetak/index-karakteristik-investasi-cetak', $data,true);  
