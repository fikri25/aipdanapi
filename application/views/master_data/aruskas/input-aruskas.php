...
<form id="form_<?=$acak;?>" method="post" url="<?php echo site_url(); ?>master-simpan/master_aruskas" enctype="multipart/form-data" >
	<input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" style="display: none">
	<input type="hidden" name="editstatus" value="<?php echo !empty($editstatus) ? $editstatus : 'add';?>">
	<input type="hidden" name="id_aruskas" value="<?php echo !empty($data) ? $data['id_aruskas'] : '';?>">

	<div class="row">
		<div class="col-md-12">
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">Form Master Arus Kas</h3>
				</div>
				<div class="box-body">
					<div class="row">
						<div class="col-md-12">
							<div class="row">
								...
								<div class="col-md-3">
									<label>Jenis Kas<font color="red">&nbsp;*</font></label>
									<select class="form-control select2nya" id="jenis_kas" name="jenis_kas">
										<option value="">
											-- Pilih Aktivitas --
										</option>
										<?php if(isset($opt_kas) && is_array($opt_kas)){?> 
											<?php foreach($opt_kas as $k=>$v){?>
												<option value="<?php echo $v['id'];?>" <?php if(!empty($data) && $v['id'] == $data['jenis_kas']) echo 'selected="selected"';?>>
													<?php echo $v['txt'];?>
												</option>
											<?php }?>
										<?php }?>
								...
								<div class="col-md-6">
									<div class="form-group">
										<label>Arus Kas<font color="red">&nbsp;*</font></label>
										<input type="text" placeholder="Arus Kas" class="form-control" id="arus_kas" name="arus_kas" value="<?php echo !empty($data) ? $data['arus_kas'] : '';?>" required="required">
									</div>
								</div>
							</div>
							<br>
		...
		submitHandler: function(form) {
			...
			submit_form('form_<?=$acak;?>',function(r){
				if(r==1){ 
					...
					setTimeout(function(){
						window.location = host+'master/master_data/master_aruskas';
					}, 2000);
				}else{ 
