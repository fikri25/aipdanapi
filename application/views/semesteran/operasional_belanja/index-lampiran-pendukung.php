                  ...
                  <div class="col-md-12">
                    <div class="tab-pane fade in" id="">
                        <form class="form-horizontal" action="<?php echo base_url().'semesteran/operasional_belanja/save_keterangan'?>" method="post" enctype="multipart/form-data">
                            ...
                            <input type="hidden" name="nmdok" value="lkob_lampiran_pendukung">
                            <input type="hidden" name="semester" value="1">
                            <div class="form-group">
                                <div class="col-sm-12">
                                    <textarea name="keterangan" class="form-control" style="height:130px;width:50%;" id="keterangan" rows="10" placeholder="Keterangan"><?php echo (isset($lampiran_lkob[0]->keterangan_lap) ? $lampiran_lkob[0]->keterangan_lap : '');?></textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label" for="keterangan">Unggah Dokumen</label>
                                <div class="col-sm-10">
                                    <input type="hidden" name="id" value="<?php echo (isset($lampiran_lkob[0]->id) ? $lampiran_lkob[0]->id : '');?>">
                                    <input type="hidden" name="filedata_lama" value="<?php echo (isset($lampiran_lkob[0]->file_lap) ? $lampiran_lkob[0]->file_lap : '');?>">
                                    <input type="file" name="filedata">
                                    <p style="margin-top:10px;">File harus bertipe pdf,doc atau docx.</p>
                                    <a href="<?php echo site_url('semesteran/operasional_belanja/get_file/'.(isset($lampiran_lkob[0]->id) ? $lampiran_lkob[0]->id : ''));?>"><p><?php echo (isset($lampiran_lkob[0]->file_lap) ? $lampiran_lkob[0]->file_lap : '');?></p></a>
                                </div>
                            </div>
                            <?php if($level != "DJA"){?>
                                <button style="height:30px; margin-bottom:10px; margin-left:5px;" class="btn btn-primary btn-md btn-flat user-smt2" type="submit">
                                    Simpan
                                </button>
                            <?php } ?>
                        </form>
